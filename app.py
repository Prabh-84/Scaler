...
# Add the parent directory to sys.path so imports work when running from server/
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
def get_tasks():
    """
    Returns available tasks and the action schema.
    """
    if not SCENARIOS:
...
def get_state():
    """
    Returns the current observation/state of the active environment.
    """
    global active_env
...
def step(req: Action):
    """
    Executes one action in the environment and returns:
