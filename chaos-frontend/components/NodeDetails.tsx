...
// import { Activity, ArrowLeft, RefreshCw, RotateCcw, TrendingUp, Eye } from 'lucide-react';
...
//         const declared = scoreData?.breakdown?.correct_root_cause;
//         const groundTruth = scoreData?.breakdown?.ground_truth_root_cause;

//         alert(
//           `EPISODE COMPLETE!\n\n` +
//           `Final Score: ${(Number(scoreData.total || 0) * 100).toFixed(1)}%\n` +
//           `Scenario: ${scoreData.scenario_id ?? "N/A"}\n\n` +
//           `Root Cause Accuracy: ${(Number(scoreData.root_cause_accuracy || 0) * 100).toFixed(1)}%\n` +
//           `Intervention Order: ${(Number(scoreData.intervention_order || 0) * 100).toFixed(1)}%\n` +
//           `Cascade Damage: ${(Number(scoreData.cascade_damage || 0) * 100).toFixed(1)}%\n` +
//           `Step Efficiency: ${(Number(scoreData.step_efficiency || 0) * 100).toFixed(1)}%\n\n` +
//           `Declared Root Cause: ${declared?.node ?? "N/A"}${declared?.failure_type ? ` (${declared.failure_type})` : ""}\n` +
//           `Ground Truth Root Cause: ${groundTruth ?? "N/A"}`
//         );
//       }
//     } catch (e) {
...
//   )
// }
const API_URL = ""; // same-domain on HF Space
...
export const api = {
  ...
  getGrader: async () => {
    ...
    if (!res.ok) throw new Error("Failed to fetch grader result");
    return res.json();
  },
};